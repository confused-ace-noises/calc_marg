...
impl Check for String {
    fn check(&self, checklen: usize) -> Result<Vec<u32>, String>{
        ...
        if it.len() > checklen {
            return Err(String::from("sono stati immessi più di") + &checklen.to_string() + "parametri/o")
        } else if it.len() < checklen {
            return Err(String::from("sono stati immessi meno di") + &checklen.to_string() + " parametri");
        } 
        ...
        for i in &it {
            ...
            if ! i.trim().chars().all(|s| s.is_digit(10)) {
                let error = "il ".to_string() + &tracker.to_string() + "o numero contiene caratteri non-numerici.";
                return Err(error)
            };
        }

        return Ok(it.into_iter().map(|s| s.parse().unwrap()).collect::<Vec<u32>>());
    }
}
